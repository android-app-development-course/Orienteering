...
public class BottomBarLayout extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    private ViewPager mViewPager;        //定义的在实例中现实的viewpager
    private int mChildCount;//子条目个数
    private List<BottomBarItem> mItemViews = new ArrayList<>();     //导航栏的子目录数组
    ...
    public BottomBarLayout(Context context, AttributeSet attrs, int defStyleAttr) {   //构造函数3
    ...
    @Override
    public void setOrientation(int orientation) {       //设置放置样式  只支持水平样式
    ...
    public void setViewPager(ViewPager mViewPager) {     //设置viewpager显示的页面
    ...
    @Override
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {   //设置页面滚动显示
